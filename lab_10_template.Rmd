...
ggplot(graph_data, aes(x = Age, y = Probability)) +
  ...
  geom_ribbon(aes(ymin = Probability - SE, ymax= Probability + SE, fill = Sex, alpah = 0.5))
...
aq_plot <- ggplot(aq_tidy, aes(x = as.numeric(year), y = aq_mil, group = country)) +
  ...
  geom_text(aes(label = country, color = country), position =  position_nudge (y = 0.04, x =1), size = 5)) + transition_reveal(country, as.numeric(year))
